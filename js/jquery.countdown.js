...
(function ($) {
  ...
  $.extend(Countdown.prototype, {
    ...
    _adjustSettings: function (target, inst) {
      ...
      if (inst._since) {
        ...
        if (inst._since && serverSync) {
          inst._since.setMilliseconds(
            inst._since.getMilliseconds() + now.getTime() - serverSync.getTime()
          );
        }
      }
      ...
      if (serverSync) {
        inst._until.setMilliseconds(
          inst._until.getMilliseconds() + now.getTime() - serverSync.getTime()
        );
      }
    ...
    _generateHTML: function (inst) {
      ...
      inst._periods = periods = inst._hold
        ? inst._periods
        : this._calculatePeriods(inst, inst._show, new Date());
      ...
      return layout
        ? this._buildLayout(inst, layout, compact)
        : (compact // Compact version
            ? '<span class="countdown_row countdown_amount' +
              ...
              (inst._show[M]
                ? (inst._show[H] ? timeSeparator : "") +
                  this._minDigits(periods[M], 2)
                : "") +
              (inst._show[S]
                ? (inst._show[H] || inst._show[M] ? timeSeparator : "") +
